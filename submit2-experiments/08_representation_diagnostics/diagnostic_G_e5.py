"""
Diagnostic G: E5-large-v2, fallback for Diagnostic F after INSTRUCTOR was
blocked by Kaggle's phone-verification/internet gate.
"""
import numpy as np
import pandas as pd
...
MODEL_PATH = "/kaggle/input/models/<fill-in-after-attaching>/e5-large-v2"

LABELS = ["Aligned", "Weakly Aligned", "Mismatched"]

# E5's ONLY supported prefixes , fixed, literal, not customizable.
QUERY_PREFIX = "query: "
...
def main():
    print("*" * 70)
    print("  Diagnostic G: E5-large-v2 with query:/passage: prefixes (v1 profile)")
    print("*" * 70)
    ...
    prefixed_chunks = [PASSAGE_PREFIX + c for c in CAPABILITY_CHUNKS]
    prefixed_items = [QUERY_PREFIX + t for t in df["item_text"].tolist()]

    print(" Encoding capability chunks with 'passage: ' prefix")
    cap_vectors = model.encode(prefixed_chunks, normalize_embeddings=True, show_progress_bar=True)
    print(" Encoding dataset items with 'query: ' prefix")
    ...
    out.to_csv("diagnostic_G_e5_results.csv", index=False)
    print("\n OK, Saved diagnostic_G_e5_results.csv")
