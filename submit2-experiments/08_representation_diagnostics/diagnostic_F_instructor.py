"""
Diagnostic F: INSTRUCTOR Custom Manual Instruction Test

Diagnosis E confirms that the failure of mismatch category recall is not due to label errors; changing
the model and adjusting the capability configuration cannot solve the problem.
Trap samples mix a large number of capability-related words into the business context. Ordinary embedding
cosine similarity only captures topic overlap and cannot distinguish whether the text truly requires
the corresponding engineering skills.
The INSTRUCTOR model can reshape the embedding vector through natural language instructions.
This script uses targeted handwritten instructions to test whether the embedding similarity framework can
distinguish between topic and skill requirements.
"""
import numpy as np
import pandas as pd
...
# frame each chunk as a reference definition of a specific hands-on engineering skill.
DOC_INSTRUCTION = (
    "Represent the engineering capability requirement, describing a specific "
    "hands-on technical skill needed to build or operate this system, for "
    "matching against candidate assessment questions:"
)

# this is the part doing the real work , it explicitly asks the model to judge whether the question
# requires the engineering skill itself, not just whether it mentions related vocabulary or occurs in a
# related business context.
QUERY_INSTRUCTION = (
    "Represent this interview question for determining whether answering it "
    "correctly requires hands-on engineering competency in the underlying "
    "technology, as opposed to business, sales, product-management, or "
    "commercial judgment that merely references the same technology by name:"
)
...
def load_instructor(model_path):
    """
        First try using the standard InstructorEmbedding package; if it's not supported,
        then use the regular SentenceTransformer."
    """
    try:

        model = INSTRUCTOR(model_path)
        return model, "instructor_embedding"
    except Exception as e:
        print(f" InstructorEmbedding load failed ({e}); trying SentenceTransformer fallback")
        local_only = model_path.startswith("/kaggle")
        model = SentenceTransformer(model_path, local_files_only=local_only)
        return model, "sentence_transformers"


def encode_with_instruction(model, backend, texts, instruction):
    if backend == "instructor_embedding":
        pairs = [[instruction, t] for t in texts]
        return np.asarray(model.encode(pairs, show_progress_bar=True))
    else:
        # Concatenate the instructions and text into a single string, then manually normalize it.
        combined = [f"{instruction} {t}" for t in texts]
        vecs = np.asarray(model.encode(combined, show_progress_bar=True))
...
def main():
    print("*" * 70)
    print("  Diagnostic F: INSTRUCTOR with targeted topic-vs-skill instruction")
    print("*" * 70)
    ...
    print(f"\n Loading {MODEL_PATH}")
    model, backend = load_instructor(MODEL_PATH)
    print(f" Loaded via backend: {backend}")

    print("\n Encoding capability chunks with DOC_INSTRUCTION")
    cap_vectors = encode_with_instruction(model, backend, CAPABILITY_CHUNKS, DOC_INSTRUCTION)
    print(" Encoding dataset items with QUERY_INSTRUCTION")
    item_vectors = encode_with_instruction(model, backend, df["item_text"].tolist(), QUERY_INSTRUCTION)

    cap_vectors = cap_vectors / np.clip(np.linalg.norm(cap_vectors, axis=1, keepdims=True), 1e-9, None)
    item_vectors = item_vectors / np.clip(np.linalg.norm(item_vectors, axis=1, keepdims=True), 1e-9, None)
    ...
    out["instructor_score"] = scores
    out["predicted_label"] = preds
    out.to_csv("diagnostic_F_instructor_results.csv", index=False)
    print("\n OK, Saved diagnostic_F_instructor_results.csv")
