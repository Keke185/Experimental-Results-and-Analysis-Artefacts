"""
    Perform symmetric binary subset content reconstruction on weakly aligned categories,
    and classify them into binary types to match the "mismatched" categories (50 product manager samples + 50 video
    conferencing sales samples).
"""
import pandas as pd

DATASET_PATH = "dataset_items.csv"
OUTPUT_PATH = "dataset_items_v2.csv"
...
REPLACEMENTS = {
    ...
    # Based on domain characteristics, the ability to retain, store, and retrieve records is preserved,
    # making session recording an inherent standard feature of video conferencing platforms
    "Q_109":
    ...
    # Directly map to Block2 "Meeting Control Policy"
    "Q_136":
    ...
    # General entries, each mapped to an approved support capability category. These entries replace those
    # initially retained but not mapped to any actual capability profile support skills

    # Below is API integration
    "Q_102":
    ...
    # Below is security & authentication
    "Q_113":
    ...
    # Below is the distributed service design
    "Q_121":
...
RTC_CONTEXT_REPLACED = {
    "Q_104", "Q_105", "Q_109", "Q_111", "Q_112", "Q_114", "Q_115", "Q_117",
    "Q_119", "Q_122", "Q_125", "Q_128", "Q_129", "Q_131", "Q_136", "Q_137",
    "Q_140", "Q_141", "Q_143", "Q_145",
}

RTC_CONTEXT_ORIGINAL = {"Q_146", "Q_147", "Q_148", "Q_149", "Q_150"}
GENERIC_MAPPED_REPLACED = {
    "Q_102", "Q_103", "Q_107", "Q_113", "Q_116", "Q_120", "Q_121", "Q_123",
    "Q_124", "Q_126", "Q_127", "Q_130", "Q_138",
}

GENERIC_MAPPED_UNCHANGED = {
    "Q_101", "Q_106", "Q_108", "Q_110", "Q_118", "Q_132", "Q_133", "Q_134",
    "Q_135", "Q_139", "Q_142", "Q_144",
}
...
def main():
    ...
    print(f"OK, Loaded {len(df)} items from {DATASET_PATH}")

    missing = [iid for iid in REPLACEMENTS if iid not in set(df["item_id"])]
    if missing:
        print(f"WARN, These item_ids were not found in the dataset and will be skipped: {missing}")

    n_changed = 0
    for item_id, new_text in REPLACEMENTS.items():
        mask = df["item_id"] == item_id
        if mask.any():
            df.loc[mask, "item_text"] = new_text
            n_changed += 1

    print(f"OK, Replaced item_text for {n_changed} items total "
          f"({len(RTC_CONTEXT_REPLACED)} RTC-context + {len(GENERIC_MAPPED_REPLACED)} generic-but-remapped)")
    print("OK, target_role and gold_label were NOT changed for any row -- schema is identical to the original")

    wa = df[df["gold_label"] == "Weakly Aligned"]
    rtc_total = wa["item_id"].isin(RTC_CONTEXT_REPLACED | RTC_CONTEXT_ORIGINAL).sum()
    generic_mapped_total = wa["item_id"].isin(GENERIC_MAPPED_REPLACED | GENERIC_MAPPED_UNCHANGED).sum()
    print(f"\n Weakly Aligned ({len(wa)} items) final composition:")
    print(f"  RTC/session-context items:            {rtc_total}")
    print(f"  Generic, capability-mapped items:      {generic_mapped_total}")
    print(f"  (sum should equal {len(wa)}: {rtc_total + generic_mapped_total == len(wa)}) "
          f"\n Every item in the 'generic, capability-mapped' half now maps to one of 8 approved "
          f"\n categories (API integration, distributed service design, troubleshooting, observability, "
          f"\n cloud deployment, security & authentication, async task processing, cross-team "
          f"\n technical coordination)  none remain purely on the basis of 'it's also software dev'")
